...
/////////////////////////////////////////////////////////////////////////////////////////
pub const MSVC_PATH: &str = env!("WRAPPER_MSVC_PATH");           // Microsoft Visual C++ toolchain path
pub const LLVM_PATH_VS: &str = env!("WRAPPER_LLVM_PATH_VS");     // Visual Studio LLVM toolchain path
pub const LLVM_PATH: &str = env!("WRAPPER_LLVM_PATH");           // Custom LLVM toolchain path
pub const GCC_PATH: &str = env!("WRAPPER_GCC_PATH");             // GNU Compiler Collection path
pub const WRAPPER_PATH: &str = env!("WRAPPER_WRAPPER_PATH");     // sccache or ccache path
...
pub static LLVM_KEYWORDS: LazyLock<Regex> = LazyLock::new(||Regex::new(r#"(?i)(clang|lld)"#).unwrap());
...
pub const ARGS_CHAR_LIMIT: usize = 30000;  // maximum CLI args char length before using response file
...
pub enum ExecutableKind {
    COMPILER,    // clang, clang-cl, cl, gcc, g++
...
pub fn print_usage() -> bool {
    ...
    lines.push(box_row(
        &center_text(
            &paint("Set any of these environment variables to enable its behavior.", ansi::DIM),
            INNER,
        ),
        INNER,
    ));
...
mod tests {
    ...
    fn splits_fused_flag_and_directory() {
        ...
        let result = filter_args(input_args, never_match(), vec![], String::new());
        assert_eq!(
            result,
            vec![
                "/Fd".to_string(),
